...
describe("multi-path extraction", () => {
  ...
  it("extracts Chinese 或 lists", () => {
    expect(scopes("禁止修改 a.py 或 b.py")).toEqual(["a.py", "b.py"]);
  });

  it("extracts Chinese 或者、和 and 、 lists", () => {
  ...
  it("honors quoted continuation paths even when they look like verbs", () => {
    expect(scopes("Never edit `core/` or `run`")).toEqual([
      "core/**",
      "run/**",
    ]);
  });
  ...
  it("extracts protect-the-following lists", () => {
    expect(scopes("Protect the following paths: src, secrets")).toEqual([
      "src/**",
      "secrets/**",
    ]);
  });

  it("extracts Chinese protect-the-following lists", () => {
    expect(scopes("保护以下文件: a.py、b.py")).toEqual(["a.py", "b.py"]);
  });
  ...
  it("denies both members of a quoted or-list", () => {
    const policy = policyFor("Never modify `config.py` or `secrets/`.");
    expect(evaluate(policy, "path", "config.py").status).toBe("denied");
    expect(evaluate(policy, "path", "secrets/.env").status).toBe("denied");
  });
});
